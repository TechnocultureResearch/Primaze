...
def addLogger(name, level):
    ...
    exec(
        "logging.Logger.{0} = partialmethod(logging.Logger.log, logging.{1})".format(
            name.lower(), name.upper()
        )
    )
    exec(
        "logging.{0} = partial(logging.log, logging.{1})".format(
            name.lower(), name.upper()
        )
    )
...
def init():
    ...
    logging.basicConfig(
        ...
        datefmt="%I:%M%p",  # %d/%m/%Y
        handlers=[logging.FileHandler("log/debug.log"), logging.StreamHandler(stdout)],
    )
